...
function($) {
...
}(window), FP.app = function() {
    function init() {
        FP.helpers.adjustImagePositioning($(el_fullScreenImage)), Modernizr.touch ? (animateIntro(), 
        bindScrollButtons(), FP.helpers.bindWindowResize(el_fullScreenImage, el_fullScreenVideo)) : (downloadAmbients(), 
        animateIntro(), initVideo(function() {
            playVideo("#start");
        }), bindScrollButtons(), FP.helpers.bindWindowResize(el_fullScreenImage, el_fullScreenVideo));
    }
    function animateIntro() {
        console.log("init"), $(".section-loading").addClass("active"), $("#intromessage li:eq(0)").fadeIn(800), 
        setTimeout(function() {
            $("#intromessage li:eq(0)").fadeOut(function() {
                $("#intromessage li:eq(1)").fadeIn(function() {
                    setTimeout(function() {
                        $("#intromessage li:eq(1)").fadeOut(function() {
                            console.log("jump to start section"), animateStart();
                        });
                    }, 2e3);
                });
            });
    ...
    function initVideo(callback) {
        videojs("mainVideo").ready(function() {
            myPlayer = this, myPlayer.loop(!0), console.log(myPlayer);
        }), FP.helpers.adjustVideoPositioning(el_fullScreenVideo), callback();
    }
    function playVideo(target) {
        ...
        myPlayer.play(), Modernizr.video && (Modernizr.touch || $(target).find(".full-screen-image").fadeOut());
    }
    function resetSection() {
        Modernizr.video && (Modernizr.touch || $(".full-screen-section:not(.active)").find(".full-screen-image").show());
    }
    function bindScrollButtons() {
        $(".article-start__btn").click(function(e) {
            var target = $(this).attr("href");
            resetSection(), Modernizr.touch || moveBGvideo(scrollDirection), goToSection(1, target, !0), 
            e.preventDefault();
        }), $(".link-list a").click(function(e) {
            $(this).addClass("active");
            var target = $(this).attr("href"), videoUrl = $(this).attr("data-video"), text = $(this).text();
            selectedVideos.push(videoUrl), console.log(selectedVideos), $(".subtitles").append("<li>" + text + "</li>"), 
            setTimeout(function() {
                Modernizr.touch || moveBGvideo(scrollDirection), goToSection(1, target, !0);
            }, 200), e.preventDefault();
        }), $(".button--play").click(function(e) {
            $(".full-screen-section.active .mega").addClass("animated fadeOutUp"), $(".full-screen-section.active .button--play").addClass("animated fadeOut"), 
            $(el_fullScreenVideo).fadeOut(2500, function() {
                myPlayer.loop(!1), myPlayer.src(""), goToSection(1, "#playingPlaylist", !1, function() {
                    initAudio(), Modernizr.touch ? console.log("mobile playlist") : (console.log("desktop playlist"), 
                    playPlaylist());
                });
            }), e.preventDefault();
        }), $("#replay").on("click", function() {
            $(".article-share").fadeOut(1e3, function() {
                goToSection(-1, "#playingPlaylist", !1, function() {
                    console.log("replay video"), playPlaylist(), $(".subtitles").show(), $(".article-message").show();
                });
            });
        });
    }
    function goToSection(steps, target, animate, callback) {
        distanceScrolled -= 100 * steps, animate ? (console.log("animate section jump"), 
        $(el_sectionContainer).transition({
            y: distanceScrolled + "%",
            easing: "custom-ease",
            duration: 800
        }, function() {
            $(".full-screen-section").removeClass("active"), $(target).addClass("active"), $(el_fullScreenVideo).css({
                transform: "translate(0, 0)"
            }), Modernizr.touch || playVideo(target), callback && "function" == typeof callback && callback();
        })) : (console.log(distanceScrolled), console.log("dont animate section jump"), 
        $(".full-screen-section").removeClass("active"), $(target).addClass("active"), $(el_sectionContainer).css({
            "-webkit-transform": "translate(0, " + distanceScrolled + "%)"
    ...
    function initAudio() {
        $mainAudio.show(), $mainAudio[0].volume = 1, $mainAudio[0].currentTime = 30.5, $mainAudio[0].play();
    }
    function playPlaylist() {
        $(el_fullScreenVideo).find("video").attr("poster", ""), $(el_fullScreenVideo).show(), 
        playPlaylistIndex(playlistCount), $("#mainVideo video").bind("ended", function() {
            playlistCount < selectedVideos.length ? (console.log(playlistCount), playPlaylistIndex(playlistCount)) : (playlistCount = 0, 
            $(".subtitles,#mainVideo").fadeOut(1200).promise().done(function() {
                goToSection(1, "#last", !1, function() {
                    console.log("going to last"), setTimeout(function() {
                        $(".article-message").fadeOut();
                    }, 3e3);
                });
            }), $("#mainVideo video").unbind("ended"), console.log("ENDED"));
        });
    }
    function playPlaylistIndex(index) {
        $(".subtitles li").hide(), $(".subtitles li:eq(" + index + ")").show(), console.log(selectedVideos[index]), 
        myPlayer.src(selectedVideos[index]), myPlayer.play(), playlistCount++;
    }
    var myAudioPlayer, myPlayer, el_fullScreenImage = ".full-screen-image", el_fullScreenVideo = "#mainVideo", el_sectionContainer = ".wrapper", $mainAudio = $("#mainAudio"), distanceScrolled = 0, scrollDirection = 1, playlistCount = 0, ambients = [], selectedVideos = [];
...
}(window), FP.helpers = function(window) {
    ...
    var mediaAspect = 16 / 9, $window = $(window);
    return {
